...
const BuildingInfo: React.FC<BuildingInfoProps> = ({
  ...
  return (
    <div className="w-full h-24 flex flex-row justify-center items-center border-b">
      <div className="w-[8%]">
        <svg
          width="20"
          height="20"
      ...
      <div className="w-[72%] h-full flex flex-col justify-center items-start">
        <span className="text-green-400 text-base font-bold">{title}</span>
        <span className="text-base text-gray-600 font-semibold">{address}</span>
      </div>
      <div className="w-[20%] h-full flex flex-col text-right justify-center py-2">
        <span className="text-base text-gray-400 font-semibold">{price}</span>
      </div>
    </div>
  );
};
