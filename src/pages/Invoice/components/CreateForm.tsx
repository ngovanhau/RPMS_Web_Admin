...
import { ServiceMeterReadings, Bill, Building, Room } from '@/types/types';
...
const CreateBillForm: React.FC<CreateBillFormProps> = ({
  ...
  useEffect(() => {
    if (serviceMeterReading) {
      setBill((prev) => ({
        ...
        date: new Date(serviceMeterReading.record_date).toISOString().split('T')[0],
        cost_service: serviceMeterReading.electricity_cost + serviceMeterReading.water_cost,
  ...
  useEffect(() => {
    if (bill.building_id) {
      const roomsForBuilding = rooms.filter(room => room.building_Id === bill.building_id);
  ...
  const handleBuildingChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    try {
      ...
      setBill(prev => ({
        ...
        roomid: '',
        roomname: '',
        cost_room: 0, // Reset cost_room when building changes
        customer_id: '',
        customer_name: '',
      }));
      setCustomerError('');
  ...
  const handleRoomChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // Find the selected room object from filteredRooms
    const selectedRoom = filteredRooms.find(room => room.id === value);

    if (selectedRoom) {
      console.log('Selected Room:', selectedRoom);
      if (!selectedRoom.customerId) {
        setCustomerError('Không có khách hàng tại phòng, vui lòng kiểm tra lại.');
      } else {
        setCustomerError('');
      }
    } else {
      console.log('No room selected or room not found.');
      setCustomerError('');
    }

    // Update the state with the selected room's ID, name, cost_room, customer_id, and customer_name
    setBill(prev => ({
      ...prev,
      [name]: value,
      roomname: selectedRoom?.room_name || '',
      cost_room: selectedRoom?.room_price || 0, // Set cost_room from room_price
      ...
      <Card className="p-6">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            />
            {errors.bill_name && <span className="text-red-500 text-sm">{errors.bill_name}</span>}
          </div>

          {/* Building Selection */}
          <div className="space-y-2">
            <Label htmlFor="building_id" className="text-red-500">Tòa nhà *</Label>
            {buildings.length > 0 ? (
              <select
                id="building_id"
                name="building_id"
                ...
                className={`w-full border rounded p-2 ${errors.building_id ? 'border-red-500' : 'border-gray-300'}`}
                required
              >
            ...
            {errors.building_id && <span className="text-red-500 text-sm">{errors.building_id}</span>}

            {/* Display Selected Building Details */}
            {selectedBuilding && (
              <div className="mt-2 p-2 border border-gray-200 rounded bg-gray-50">
                <p><strong>Địa chỉ:</strong> {selectedBuilding.address}, {selectedBuilding.district}, {selectedBuilding.city}</p>
                <p><strong>Số tầng:</strong> {selectedBuilding.number_of_floors}</p>
                <p><strong>Mô tả:</strong> {selectedBuilding.description}</p>
          ...
          {/* Room Selection */}
          <div className="space-y-2">
            <Label htmlFor="roomid" className="text-red-500">Phòng *</Label>
            {bill.building_id ? (
              filteredRooms.length > 0 ? (
                <select
                  ...
                  className={`w-full border rounded p-2 ${errors.roomid ? 'border-red-500' : 'border-gray-300'}`}
                  required
                >
            ...
            {errors.roomid && <span className="text-red-500 text-sm">{errors.roomid}</span>}
            {customerError && <span className="text-red-500 text-sm">{customerError}</span>}
          </div>
          ...
          {/* Cost Room */}
          <div className="space-y-2">
            <Label htmlFor="cost_room" className="text-red-500">Giá phòng (VNĐ) *</Label>
            <Input
              id="cost_room"
              name="cost_room"
              type="number"
              ...
              className={`bg-gray-100 border rounded p-2 ${errors.cost_room ? 'border-red-500' : 'border-gray-300'}`}
              placeholder="Giá phòng tự động điền từ phòng đã chọn"
              required
            />
            {errors.cost_room && <span className="text-red-500 text-sm">{errors.cost_room}</span>}
          </div>
