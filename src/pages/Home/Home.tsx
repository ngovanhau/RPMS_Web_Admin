...
import { getAllRoom } from "@/services/buildingApi/buildingApi";
...
const DashboardHome: React.FC = () => {
  ...
  const [roomHaveCustomerList, setRoomHaveCustomerList] = useState<Room[]>([]);
  useEffect(() => {
    const fetchDashboard = async () => {
      setLoading(true);
      try {
        const data = await getAllDashboard();
        const responseAllRoom = await getAllRoom();
        setAllRoom(responseAllRoom?.data.data);
        const responseRoomHaveCustomer = await getroombystatus(1);
        setRoomHaveCustomerList(responseRoomHaveCustomer.data);
  ...
  const summaryItems = [
    {
      title: "Tòa nhà",
      value: dashboardData?.building || 0,
      icon: <FaBuilding />,
      color: "text-blue-500",
    },
    {
      title: "Khách hàng",
      value: dashboardData?.customer || 0,
      icon: <FaUsers />,
      color: "text-green-500",
    },
    {
      title: "Hợp đồng",
      value: dashboardData?.contract || 0,
      icon: <FaFileContract />,
      color: "text-yellow-500",
    },
    {
      title: "Vấn đề",
      value: dashboardData?.problem || 0,
      icon: <FaExclamationTriangle />,
      color: "text-red-500",
    },
    {
      title: "Phòng",
      value: dashboardData?.room || 0,
      icon: <FaDoorClosed />,
      color: "text-cyan-500",
    },
  ];
  ...
  return (
    <div className="container mx-auto p-6 mt-12">
      ...
      <div className="mt-8">
            <RoomOccupancyChart
              allRoom={allRoom}
              roomHaveCustomerList={roomHaveCustomerList}
            />
        </div>
